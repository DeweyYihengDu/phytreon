"""Empirical amino-acid replacement models: JTT, WAG, LG.

Each model is a symmetric 20x20 exchangeability matrix ``R`` (off-diagonal
rates, before scaling by equilibrium frequencies) plus an equilibrium
frequency vector ``pi``, in the standard PAML amino-acid order::

    A R N D C Q E G H I L K M F P S T W Y V

The raw numbers below are transcribed verbatim from the PAML distribution's
``dat/`` files (https://github.com/abacus-gene/paml/tree/master/dat), parsed
programmatically rather than hand-copied into a matrix literal, to avoid
transcription errors in 190+ significant scientific constants:

* WAG -- Whelan, S. and N. Goldman. 2001. A general empirical model of
  protein evolution derived from multiple protein families using a maximum
  likelihood approach. Mol. Biol. Evol. 18:691-699.
* LG  -- Le, S. Q., and O. Gascuel. 2008. An improved general amino acid
  replacement matrix. Mol. Biol. Evol. 25:1307-1320.
* JTT -- Jones, D.T., Taylor, W.R. and Thornton, J.M. 1992. The rapid
  generation of mutation data matrices from protein sequences. CABIOS
  8:275-282 (PAML's ``jones.dat``, fit to SWISS-PROT 22 by Z. Yang, 1995).

Correctness is checked in ``tests/test_protein_ml.py`` by comparing the
log-likelihood our pruning algorithm computes against IQ-TREE2 on the same
fixed tree/alignment/model (independent implementation, same published
matrices) -- not just internal self-consistency.
"""
...
def _parse_paml_dat(lower_triangle: str, freqs: str):
    """Lower-triangular S_ij rows (PAML .dat layout) + frequency line(s)
    -> (20x20 symmetric list-of-lists, 20-element frequency list)."""
    rows = [[float(x) for x in line.split()] for line in lower_triangle.strip("\n").splitlines()]
    n = len(rows) + 1
    R = [[0.0] * n for _ in range(n)]
    for i, row in enumerate(rows, start=1):
        for j, v in enumerate(row):
            R[i][j] = R[j][i] = v
    pi = [float(x) for x in freqs.split()]
    if len(pi) != n:
        raise ValueError(f"expected {n} frequencies, got {len(pi)}")
    return R, pi


# ---------------------------------------------------------------------------
# WAG (Whelan & Goldman 2001) -- paml/dat/wag.dat
# ---------------------------------------------------------------------------
_WAG_S = """
0.551571
0.509848  0.635346
...
# ---------------------------------------------------------------------------
# LG (Le & Gascuel 2008) -- paml/dat/lg.dat
# ---------------------------------------------------------------------------
_LG_S = """
0.425093
0.276818 0.751878
...
# ---------------------------------------------------------------------------
# JTT (Jones, Taylor & Thornton 1992) -- paml/dat/jones.dat
# ---------------------------------------------------------------------------
_JTT_S = """
58
54  45
...
WAG_R, WAG_PI = _parse_paml_dat(_WAG_S, _WAG_PI)
LG_R, LG_PI = _parse_paml_dat(_LG_S, _LG_PI)
JTT_R, JTT_PI = _parse_paml_dat(_JTT_S, _JTT_PI)
